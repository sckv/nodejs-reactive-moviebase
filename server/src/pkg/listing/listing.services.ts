...
export const ListingServices = async (mc?: Db) => {
  ...
  return {
    ...
    delete: (listId: ObjectId | MongoObjectID, selfId: ObjectId | MongoObjectID) => {
    ...
    addMovie: ({
      ...
      return ListsRepo.addMovie({...other, listId: createObjectId(listId), selfId: createObjectId(selfId)});
    },
    removeMovie: ({
      listId,
      selfId,
