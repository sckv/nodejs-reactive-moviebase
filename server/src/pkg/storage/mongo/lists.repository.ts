...
export const ListsRepository = (db: Db) => {
  return {
    ...
    addMovie: async ({ commentary, listId, movieId, rate, selfId }: AddMovieToListObject): Promise<boolean> => {
      let findRate: any = await db
        .collection('movies')
        .findOne<{ _id: ObjectId }>({ _id: movieId, 'ratedBy.userId': selfId }, { projection: { _id: 1 } });

      if (!findRate)
        findRate = await db.collection('movies').updateOne(
          { _id: movieId },
          {
            $addToSet: {
              ratedBy: {
      ...
      if (!findRate && !findRate.modifiedCount) {
